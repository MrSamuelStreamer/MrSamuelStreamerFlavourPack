...
**Last Updated**: 2026-04-06 — all HIGH/CRITICAL issues resolved; 4 MEDIUM/LOW deferred/documented remain
...
| [007](ISSUE_007_genetron_fragile_iterator_transpilers.md) | HIGH | Harmony Patches | 9 fragile iterator transpilers | `Building_Genetron_Patch.cs` | ✅ FIXED |
| [008](ISSUE_008_transferable_ui_full_method_replace.md) | HIGH | Harmony Patches | Full method replacement via Prefix | `TransferableUIUtility_...Patch.cs` | 📝 DOCUMENTED |
...
| [012](ISSUE_012_vfe_copy_pasted_positional_args.md) | HIGH | C# / Architecture | Copy-pasted VEF code with 30 positional args | `HediffComp_TweakedAsexualReproduction.cs` | ✅ FIXED |
...
| [015](ISSUE_015_settings_tabs_static.md) | MEDIUM | Mod Architecture | Tabs list is static, shared across instances | `Settings.cs` | ✅ FIXED |
| [016](ISSUE_016_duplicate_settings_serialization.md) | MEDIUM | Mod Architecture | Duplicate field serialization | `Settings.cs` | ✅ FIXED |
...
| [022](ISSUE_022_linq_in_tick_paths.md) | MEDIUM | Performance | LINQ allocations in tick/update paths | Multiple files | ✅ FIXED |
...
| [040](ISSUE_040_bodyhophaunt_scribe_values_pawninfo.md) | HIGH | C# Code Quality | Scribe_Values used for IExposable PawnInfo — lost on save/load | `HediffComp_BodyHopHaunt.cs` | ✅ FIXED |
...
|---|---|---|
| 008 | TransferableUIUtility full replace | Prefix replacement retained by design — adding a UI column cannot be done via Postfix or Transpiler without equal fragility. Risk accepted and documented. |
| 018 (partial) | Uncached DefDatabase lookups | Several tick-path lookups remain uncached pending profiling to confirm they're hot. |
| 025 | Binge patch too broad | Intentional design decision to enable all chemicals; documented and retained. |
...
All CRITICAL and HIGH issues are resolved. Remaining items are low-priority or documented by design:

1. **TransferableUIUtility Prefix** (008) -- retained by design; update this file manually after any RimWorld `DoRow` change.
2. **Uncached DefDatabase lookups** (018 partial) -- profile before optimising; only fix paths confirmed hot.
3. **Binge patch broad enable** (025) -- documented design decision; revisit if players report unexpected behaviour.
4. **HediffSwitchMapComponent comp swap** (032 partial) -- low-priority; state preservation requires a more invasive refactor.
