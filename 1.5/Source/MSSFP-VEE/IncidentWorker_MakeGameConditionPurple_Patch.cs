...
{
    public static Lazy<MethodInfo> SendStandardLetter = new Lazy<MethodInfo>(()=>AccessTools.Method(
    ...
    {
        if(!__instance.def.HasModExtension<IncidentDefModExtension>()) return;

        IncidentDefModExtension defModExtension= __instance.def.GetModExtension<IncidentDefModExtension>();

        if (defModExtension.extraConditions.NullOrEmpty()) return;
        ...
        foreach (GameConditionDef extraGameCondition in defModExtension.extraConditions)
        {
            ...
            SendStandardLetter.Value.Invoke(__instance, [(TaggedString) extraGameCondition.label, (TaggedString) extraGameCondition.letterText, extraGameCondition.letterDef, parms, LookTargets.Invalid, Array.Empty<NamedArgument>()]);
        }
