...
public class HediffCompOutpostSuppression : HediffComp
{
    ...
    {
        if (Props.OutpostDefs.NullOrEmpty() || parent.pawn is not { Map: null })
            return;
        if (!parent.pawn.IsHashIntervalTick(GenDate.TicksPerDay))
            return;

        if (!Find.World.worldObjects.AllWorldObjects.Where(wo => Props.OutpostDefs.Contains(wo.def)).Any(wo => wo is Outpost o && o.AllPawns.Contains(parent.pawn)))
            return;
