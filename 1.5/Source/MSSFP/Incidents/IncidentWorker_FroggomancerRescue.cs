...
public class IncidentWorker_FroggomancerRescue: IncidentWorker
{
    ...
    {
        if (FroggomancerKind == null && MSSFP_FroggoHeal != null) return false;
        if (parms.target is not Map map) return false;
        if (!map.mapPawns.SpawnedDownedPawns.Any(p => p.Faction == Faction.OfPlayer)) return false;

        Pawn targetPawn = map.mapPawns.SpawnedDownedPawns.Where(p => p.Faction == Faction.OfPlayer).RandomElement();

        Pawn froggomancer = PawnGenerator.GeneratePawn(new PawnGenerationRequest(
            kind: FroggomancerKind,
            forceGenerateNewPawn: true
            ));

        IEnumerable<IntVec3> cells = GenRadial.RadialCellsAround(targetPawn.Position, 10, true).Where(cell=>map.reachability.CanReach(cell, targetPawn, PathEndMode.InteractionCell, TraverseMode.ByPawn, Danger.Deadly));
