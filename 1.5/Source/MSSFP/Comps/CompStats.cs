...
public class CompStats: ThingComp
{
    ...
    {
        if(Props.factors.NullOrEmpty()) return base.GetStatFactor(stat);
        StatModifier factor = Props.factors.FirstOrDefault(o => o.stat == stat);
        if(factor == null) return base.GetStatFactor(stat);
        return base.GetStatOffset(stat) * factor.value;
    }

    public override float GetStatOffset(StatDef stat)
    {
        if(Props.offsets.NullOrEmpty()) return base.GetStatOffset(stat);
        StatModifier offset = Props.offsets.FirstOrDefault(o => o.stat == stat);
        if(offset == null) return base.GetStatOffset(stat);
