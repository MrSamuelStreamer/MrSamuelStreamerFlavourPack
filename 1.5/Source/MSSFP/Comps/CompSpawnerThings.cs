      ...
      out IntVec3 result)
    {
      foreach (IntVec3 intVec3 in GenAdj.CellsAdjacent8Way(parent).InRandomOrder())
      {
        if (intVec3.Walkable(parent.Map))
        {
          ...
          if ((edifice == null || !thingToSpawn.IsEdifice()) && (!(edifice is Building_Door buildingDoor) || buildingDoor.FreePassage) && (parent.def.passability == Traversability.Impassable || GenSight.LineOfSight(parent.Position, intVec3, parent.Map)))
          {
            bool flag = false;
