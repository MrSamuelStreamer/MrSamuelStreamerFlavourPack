...
public class RitualPosition_Sit : RitualPosition
{
    ...
    public override PawnStagePosition GetCell(IntVec3 spot, Pawn p, LordJob_Ritual ritual)
    {
        ...
        if (
            ritual.selectedTarget != null
            && ritual.selectedTarget.Thing.def.defName == "MSSFP_CuckChair"
        )
            return new PawnStagePosition(
                ritual.selectedTarget.Thing.Position,
                null,
                Rot4.Invalid,
                highlight
            );
