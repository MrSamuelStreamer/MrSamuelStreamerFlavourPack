...
public class Verb_CastAbilityWithCost : Verb_CastAbility
{
    public virtual DamageWorker.DamageResult DamageBrain()
    {
        ...
        DamageInfo dinfo = new DamageInfo(DamageDefOf.Scratch, 1f, 1f, -1f, CasterPawn, brain, spawnFilth: true);
    ...
    protected override bool TryCastShot()
    {
        ...
        if (skillRecord.xpSinceLastLevel >= (xpForCurrentLevel * 0.10f))
        {
            if (CasterPawn.Faction.IsPlayer)
                Messages.Message("MSSFP_LostBrainPower".Translate(CasterPawn.NameShortColored, skillRecord.def.LabelCap), CasterPawn, MessageTypeDefOf.NegativeEvent, true);
        ...
        {
            if (CasterPawn.Faction.IsPlayer)
                Messages.Message("MSSFP_LostBrainPowerDamaged".Translate(CasterPawn.NameShortColored, skillRecord.def.LabelCap), CasterPawn, MessageTypeDefOf.NegativeEvent, true);
            DamageBrain();
        }
